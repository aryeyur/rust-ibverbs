...
impl QueuePairBuilder {
    ...
    pub fn build(&self) -> io::Result<PreparedQueuePair> {
        let mut attr = ffi::ibv_qp_init_attr {
            ...
            srq: self.srq.as_ref().map(|s| s.inner.srq).unwrap_or(ptr::null::<ffi::ibv_srq>() as *mut _),
            cap: ffi::ibv_qp_cap {
...
impl ProtectionDomain {
    ...
    pub fn create_srq(&self, max_wr: u32, max_sge: u32, srq_limit: u32) -> io::Result<SharedReceiveQueue> {
...
impl SharedReceiveQueue {
    ...
    pub unsafe fn post_receive(
        ...
        let errno = unsafe {
            ops.post_srq_recv.as_mut().unwrap()(self.inner.srq, &mut wr as *mut _, &mut bad_wr as *mut _)
        };
        if errno != 0 {
...
mod test {
    ...
    fn test_srq_creation() {
        ...
        let ctx = match dev.open() {
            Ok(ctx) => ctx,
            Err(_) => return, // May fail if no active port or permissions
        };
        let pd = ctx.alloc_pd().unwrap();
        let srq = pd.create_srq(10, 1, 0).unwrap();
        assert!(!srq.inner.srq.is_null());

        let cq = ctx.create_cq(10, 0).unwrap();
        let qp_builder = pd
            .create_qp(&cq, &cq, ffi::ibv_qp_type::IBV_QPT_RC)
            .unwrap()
            .set_srq(&srq)
            .build();
        assert!(qp_builder.is_ok());
    }
}
